...
model_t	pKnownModels[MAX_MOD_KNOWN];
int		iCurrentKnownModelCount;
...
void Mod_ClearAll(void)
{
   ...
   for (i = 0, mod = pKnownModels; i < iCurrentKnownModelCount; i++, mod++) {
...
==================
*/
model_t* Mod_FindName(char* szName)
{
   int		i;
   model_t* pCurrentModel;
   model_t* pAvailable = NULL;
   ...
   //
   for (i = 0, pCurrentModel = pKnownModels; i < iCurrentKnownModelCount; i++, pCurrentModel++)
   {
      if (!strcmp(pCurrentModel->name, szName))
         break;
      if (pCurrentModel->needload == NL_UNREFERENCED)
         if (!pAvailable || pCurrentModel->type != mod_alias)
            pAvailable = pCurrentModel;
   }

   if (i == iCurrentKnownModelCount)
   {
      if (iCurrentKnownModelCount == MAX_MOD_KNOWN)
      {
         if (pAvailable)
         {
            pCurrentModel = pAvailable;
            if (pCurrentModel->type == mod_alias)
               if (Cache_Check(&pCurrentModel->cache))
                  Cache_Free(&pCurrentModel->cache);
         }
         else
            Sys_Error("iCurrentKnownModelCount == MAX_MOD_KNOWN");
      }
      else
         iCurrentKnownModelCount++;
      strcpy(pCurrentModel->name, szName);
      pCurrentModel->needload = NL_NEEDS_LOADED;
   }
...
==================
*/
// AngryCPPCoder: Load model usng name
