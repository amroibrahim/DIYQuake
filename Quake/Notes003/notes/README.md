# Notes 003 - PAK Files  
After analyzing Quake memory manager, it is time to read some data and load it into memory. Quake assets are grouped into ```PAK``` files, due to the naming I was always under the impressions it was compressed, but the is not the case. PAK file has a lot in common with DOOM's WAD file format.  
...
Before we can read PAK files, we need to find them first! Quake code start looking for PAK files as part of the common functionality initialization ```void COM_InitFilesystem(void)```. By default, Quake looks in current working directory (Current EXE location), but this behavior can be modified by passing ```-basedir``` as a parameter to the EXE.  
...
Searching for pak files in a folder is accomplished by looking for ```pakX.pak``` where X is an incremented each iteration starting from zero. So, it would look for PAK0.pak, PAK1.pak, PAK2.pak and so on. It would only stop if it can't load the sequenced PAK file. That means you must have the packs numbers in sequence with no gaps in numbering or else the PAK will not be loaded. With each pack file found a new ```searchpath_t``` node is added to the search liked list.  
...
Though all the nodes in the linked list are of type of ```searchpath_t``` which are set differently, a node holding data about a directory, or a node holding data about a pak file. A directory node is a node that has a ```filename``` set with a directory path and had its ```pack_t*``` pointing to null (last node in above diagram). A pak file node has a ```filename``` set to null and ```pack_t*``` pointing to a ```pack_t``` object.  

Note: The use case for this is to look for a file in PAK files and if not found it looks in the directory, this is usefully for modding a debugging. So if your testing with a new game assets no need to put them in a PAK file, just drop them in the PAK directory.  

The found pack files have their own defining node  
...
Let look at ```packfile_t* files```, this struct holds information about the files that are packed in the pak file  
...
## Pack File
Pack files are a simple format that groups files in a single big file, (similar to DOOMs WAD). As most formats, PAK files have a header that identifies the file and points to where you can read the directory list.  

The header is formatted as the following  
...
After reading the header we know where the dictionary offset is, and we can extract information about individual files in the PAK, each directory entry in the PAK is structures as following  
...
2. If file is open it reads the file header  
3. Validates that the header name is set to "PACK"  
4. Calculate the number of files (dirlen / sizeof dpackfile_t struct)  
5. Check number of files in pack (MAX is 2048 files in a single pack)  
6. Check number of files in pack match official PAK0 (to load modified PAKs you need a registered version)  
...
## Find Files
Now that we have the PAK files directory in memory, let’s have a look at how Quake search for a file in those structures. The function ```int COM_FindFile(char* filename, int* handle, FILE** file)``` which is in ```common.c``` accomplish that and takes three arguments, the first argument is the filename to searched for, the argument is an input that must be passed to the function, but the other two argument are output argument. The caller of the function ```COM_FindFile``` has a choice of passing ether a valid handle or a valid file pointer (but not both). If he passed a valid ```int* handle``` it is populated with the files handler index if the file is found or set to null if file is not found. If a valid ```FILE** file``` is passed the function it would "re-open" the pack file and return the new file handler (yes re-open, since we already have an all the pack files open and stored their handlers in ```sys_handles``` array). When the searched files are found its size is returned.   

Note: Opening a file passing a valid ```FILE** file```, is used in demo play, we will investigate that in some other notes.  
 
The main idea is liner search though each pack file or directory in the search list ```com_searchpaths```.  
...
Finally, I would like to note that in memory the search list stores the pack files in reverse order, that mean PAK2 will show up in the search list before PAK1, and PAK1 will show before PAK0. This is very important for patching and game mods, if you want to overwrite a file in PAK0, just release an updated version in PAK1 and leave PAK0 as is (searching for files is sequential).  

## Coding
First thing to do is implement searching for PAK files, so to make things simple and easy I wanted to implement "-basedir" parameter passing similar to what is implemented in Quake. The parameters allow the user to specify a PAK file path (it is very handy for debugging and molding). The Parameters struct simply stores the parameters passed to the EXE for us to use later as needed.
...
Create the create the structures required to hold the data as discusses in the notes
...
That is not bad, next we need to read some data from the PAK file, and see if we can make use of it.
