...
Cache::Cache() : m_pMemoryManager(nullptr)
{
}
...
void* Cache::NewNamed(CacheData* pCacheData, int32_t iSize, std::string& sName)
{
   ...
   while (true)
   {
      ...
      if (pCacheNode)
      {
         strncpy_s(pCacheNode->szName, sName.c_str(), sizeof(pCacheNode->szName) - 1);
   ...
   return GetIfCached(pCacheData);
}

void* Cache::GetIfCached(CacheData* pCacheData)
{
   // Is data in cache?
   if (!pCacheData->pData)
      return nullptr;
